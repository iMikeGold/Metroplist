...
def generate_sql(m49: list[dict[str, str]], capitals: list[dict[str, object]], wpp: list[dict[str, str]], hashes: dict[str, str]) -> str:
    ...
    for row in wpp:
        ...
        lines.append(f"INSERT OR IGNORE INTO observations (id,geography_id,indicator_id,unit_id,dataset_release_id,value_numeric,reference_year,reference_period_start,reference_period_end,publication_date,ingested_at,verified_at,quality_status,preferred_status,is_estimate,methodology_version) VALUES ({q(raw_id)},{q(geo_id)},'ind_wpp_population_thousands','unit_thousand_people','rel_wpp_2024',{raw},{year},{q(str(year)+'-07-01')},{q(str(year)+'-07-01')},'2024-07-11',{q(RETRIEVED_AT)},{q(RETRIEVED_AT)},'verified','preferred',1,'WPP 2024 medium estimate, source unit');")
        ...
        lines.append(f"INSERT OR IGNORE INTO observations (id,geography_id,indicator_id,unit_id,value_numeric,reference_year,reference_period_start,reference_period_end,publication_date,ingested_at,verified_at,quality_status,preferred_status,is_estimate,methodology_version) VALUES ({q(output_id)},{q(geo_id)},'ind_population_total','unit_people',{raw * 1000.0},{year},{q(str(year)+'-07-01')},{q(str(year)+'-07-01')},'2024-07-11',{q(RETRIEVED_AT)},{q(RETRIEVED_AT)},'verified','preferred',1,'Metroplist WPP unit conversion v1');")
